...
header-includes:
- "`\\usepackage[a4paper, margin=2cm]{geometry}`{=latex}"
- "`\\usepackage{minted}`{=latex}"
- "`\\usepackage{fontspec}`{=latex}"
- "`\\setmonofont{Iosevka}`{=latex}"
- "`\\setminted{fontsize=\\small, frame=single, breaklines=true}`{=latex}"
- "`\\usemintedstyle{emacs}`{=latex}"
...
$V_p = 10V$ it takes $t = 11s$ to fill the tank to $h = 20cm$. This data
allows us to calculate the pump gain.
...
\end{align*}
```
If we set the pump voltage to $V_p = 5V$ it takes $t = 17.41s$ to fill
the tank to $h = 20cm$. Once again, this data allows us to calculate the
pump gain.
...
:TODO: *Record the estimated $C_d$ for each tank:* :NOTE: During the lab
the we had Zeke attempt this too and he also could not calculate it. We
were getting a value that was ?greater than one?, which is impossible
...
:DONE: Experimental: This table was filled in using the code seen in
:TODO: Appendix X NOTE: Copy code from matlab into appendix, just the
functions
...
Using operating point three (7V center). Use the plot from matlab to estimate second order parameters. 

a) Estimate $K$,$\zeta$ and $\omega_n$ for a prototype second order transfer function from your measured step response metrics.
...
b) Use MATLAB's Control Systems Toolbox system identification app to estimate a second order transfer function based on your step response data.
:TODO: **Selected operating point initial and final value for $V_P(t)$:**
Using operating poing 3(same as last part).
...
:TODO: add image here of the comparison betwwen the two modles.
...
**Selected operating point initial and final value for $V_p(t)$:**
Select the operating point centered at 9V.
